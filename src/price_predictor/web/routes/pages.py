...
async def history(request: Request) -> HTMLResponse:
    """Prediction history — placeholder for v1, real impl in Step 2."""
    # Step 1 placeholder so the nav link works. Step 2 will render the
    # actual history table from SQLite.
    return templates.TemplateResponse(
        request=request,
        name="pages/home.html",  # reuses home for now; gets its own template in Step 2
        context={"app_version": APP_VERSION},
    )
